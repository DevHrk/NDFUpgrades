...
public class MessageSenderUtils {
	...
	public static void send(CommandSender sender, Object... rawmsg) {
	    Stream.of(rawmsg)
	       ...
	       .map(o -> (String) o)
	       .forEach(msg -> processMessage(sender, msg));
	}
	...
	enum MessageHandlers implements MessageHandler {
	    ...
	    ACTION_BAR("actionbar: ", (sender, message) -> ActionBarAPI.sendActionBarMessage((Player) sender, message.replace("actionbar: ", ""))),
	    TITLE("title: ", (sender, message) -> TitleAPI.sendTitle((Player) sender, 20, 30, 20, message.replace("title: ", ""), "")),
