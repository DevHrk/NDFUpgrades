...
public class Fragmento extends Commands {
	...
	public void perform(CommandSender s, String lb, String[] args) {
	    ...
	    if (!s.hasPermission(ps.getString("givefragmento"))) {
	    	MessageSenderUtils.send(s, m1.getString("mensagem.semperm"),m1.getStringList("mensagem.semperm"));
	        return;
	    }
	    if (args.length < 2) {
	    	MessageSenderUtils.send(s, m1.getString("Comando-Errado.fragmento") ,m1.getStringList("Comando-Errado.fragmento"));
	        return;
	    } 
	    if (args.length == 2) {
	        ...
	        ItemStack item3 = new ItemStack(Material.INK_SACK, quantia);
	        ItemMeta i3 = item3.getItemMeta();
	        item3.setDurability((short) 11);
	        i3.setDisplayName(m.getString("Fragmento.Nome").replace("&", "§"));
	        ...
	        MessageSenderUtils.send(s, m1.getString("Fragmentos.enviado").replace("{player}", targetPlayer.getName()).replace("{quantia}", FormatterAPI.formatter(quantia)) ,m1.getStringList("Fragmentos.enviado").stream().map(message -> message.replace("{player}", targetPlayer.getName()).replace("{quantia}", FormatterAPI.formatter(quantia))).collect(Collectors.toList()));
	    }
	}
}
